...
class FunctionResolver(
   ...
   *  - If the function is unqualified or builtin-qualified `count(*)` (DataFrame API), replace
   *    it with `count(1)` via [[normalizeCountExpression]]; otherwise resolve children of the
   *    function.
