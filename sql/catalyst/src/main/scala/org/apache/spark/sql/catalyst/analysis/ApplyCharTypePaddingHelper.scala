...
object ApplyCharTypePaddingHelper {
  ...
  private[sql] def readSidePadding(
    ...
    readSidePadding(
      relation,
      cleanedRelation,
      standardSemantics = SQLConf.get.charVarcharStandardSemantics)
  }

  private[sql] def readSidePadding(
      relation: LogicalPlan,
      cleanedRelation: () => LogicalPlan,
      standardSemantics: Boolean): (LogicalPlan, Seq[(Attribute, Attribute)]) = {
    val projectList = relation.output.map { attr =>
      CharVarcharUtils.addPaddingForScan(attr, standardSemantics) match {
   ...
   * helper for `relation`. This deliberately rejects user projections and projections over any
   * plan other than the direct relation.
   */
